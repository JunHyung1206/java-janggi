...
class GameTest {
    ...
    void 유효한_기물_이동이라면_성공한다() {
        Board board = new Board(List.of(new Chariot(Team.CHO, new Position(0, 0))));
        Game game = new Game(board);
        assertDoesNotThrow(() -> game.move(new Position(0, 0), new Position(5, 0)));
    }
    ...
    void 궁이_둘다_죽지_않았다면_결과를_받을_수_없다() {
        Board board = new Board(List.of(
                ...
                new Cannon(Team.HAN, new Position(1, 0)),
                new General(Team.HAN, new Position(4, 9)),
                new General(Team.CHO, new Position(4, 0))
        ));
        Game game = new Game(board);
